    ...
    public TeacherReadOnlyDTO insertTeacher(TeacherInsertDTO insertDTO) throws EntityAlreadyExistsException, EntityInvalidArgumentException {
        try {
            ...
            if(teacherDAO.getByVat(insertDTO.getVat()).isPresent()) {
                throw new EntityAlreadyExistsException("Teacher", "Teacher with vat: " + insertDTO.getVat() + " not inserted");
            }
    ...
    public TeacherReadOnlyDTO updateTeacher(TeacherUpdateDTO updatedto) throws EntityNotFoundException, EntityInvalidArgumentException {
        try {
            ...
            if(teacherDAO.getByVat(updatedto.getVat()).isEmpty()) {
                throw new EntityNotFoundException("Teacher","Teacher with vat "+updatedto.getVat()+" not inserted");
            }

            TeacherReadOnlyDTO readOnlyDTO = teacherDAO.update(teacher)
                    .map(Mapper::mapToTeacherReadOnlyDTO)
                    .orElseThrow(() -> new EntityInvalidArgumentException("Teacher","Teacher with vat "+updatedto.getVat()+" not inserted"));
            ...
            JPAHelper.rollbackTransaction();
            LOGGER.error("Error. Teacher not updated: id{}, vat{} firstname: {} , lastname {}",
                    updatedto.getId(), updatedto.getVat(), updatedto.getFirstname(), updatedto.getFirstname());
            throw e;
        } finally {
            JPAHelper.closeEntityManager();
        }
